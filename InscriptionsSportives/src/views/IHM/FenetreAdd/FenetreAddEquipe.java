...
public class FenetreAddEquipe extends FenetreAdd {

	private static final long serialVersionUID = -2608641548957721950L;

	FenetreListPersonne personneClass = new FenetreListPersonne(new Application(), this);

	JLabel lblAjouter = new JLabel("Créer l'équipe");
	...
	public FenetreAddEquipe(Application application, FenetreMain fenetreParent) {

		super(application, fenetreParent);

		initializeWindow();
		getHeader();
		getTopPanel("Ajouter une équipe");
		getMainPanelContent();
		getFooter();
		getListener();
	...
	private void getMainPanelContent() {
		...
		containerInfoPanelBottomRight.add(lblAjouter);
	...
	protected void getListener() {
		...
		lblAjouter.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseEntered(MouseEvent e) {
				lblAjouter.setBackground(Color.decode("#2ecc71").darker());
			}
			@Override
			public void mouseExited(MouseEvent e) {
				lblAjouter.setBackground(Color.decode("#2ecc71"));
		...
		lblAddMember.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseEntered(MouseEvent e) {
				lblAddMember.setBackground(Color.decode("#27AC5F"));
				checkButton(true);
			}
			@Override
			public void mouseExited(MouseEvent e) {
	...
	private void ajouterAction() {
		...
		Equipe createdEquipe = application.createEquipe(nomEquipeValue);

		for(Personne personne : equipePersonnes ){
			createdEquipe.add(personne);
		}
