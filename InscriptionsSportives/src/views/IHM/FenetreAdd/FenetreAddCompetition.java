...
public class FenetreAddCompetition extends FenetreAdd {

	private static final long serialVersionUID = 7270726886751706219L;
	...
	JLabel lblAjouter = new JLabel("Créer la compétition");
	...
	public FenetreAddCompetition(Application application, FenetreMain fenetreParent) {

		super(application, fenetreParent);

		initializeWindow();
		getHeader();
		getTopPanel("Ajouter une Compétition");
		getMainPanelContent();
		getFooter();
		getListener();
	...
	private void getMainPanelContent() {
		...
		lblAjouter.setBorder(new EmptyBorder(10, 25, 10, 25));
		lblAjouter.setBackground(new Color(46, 204, 113));
		lblAjouter.setAlignmentX(0.5f);
		containerInfoPanelLeftBottom.add(lblAjouter);
	...
	private void ajouterAction() {
		...
		Competition createdCompetition = application.createCompetition(nomCompetitionValue, dateClotureCompetitinValue, equipeCompetitionValue);

		if( rdbtnComptitionEnEquipe.isSelected() ){
			for(Equipe equipe : competitionCandidatEquipes ){
				createdCompetition.add(equipe);
			}
		}else if( rdbtnComptitionEnSolo.isSelected() ){
			for(Personne personne : competitionCandidatPersonnes ){
				createdCompetition.add(personne);
			}
		}
